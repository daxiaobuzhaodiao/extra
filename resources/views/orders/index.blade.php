                    ...
                    <div class="card-body">
                        ...
                        @foreach($order->orderItems as $index => $item)
                            ...
                            @if($index === 0)
                                ...
                                @else
                                    未支付<br>
                                    请于 {{ $order->created_at->addSeconds(config('app.order_ttl'))->format('m-d H:i') }} 前完成支付<br>
                                    否则订单将自动关闭
                                @endif
                                </td>
                                <td rowspan="{{ count($order->orderItems) }}" class="text-center"><a class="btn btn-primary btn-sm" href="{{ route('orders.show', $order->id) }}">查看订单</a></td>
                            @endif
                            </tr>
                        @endforeach
                        </table>
